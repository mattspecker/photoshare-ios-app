...
class WebsiteIntegration {
  ...
  async getAuthToken() {
    try {
      // First try to get token from PhotoShare auth bridge if available
      if (window.getPhotoShareJwtTokenForAndroid && typeof window.getPhotoShareJwtTokenForAndroid === 'function') {
        console.log('🔑 Getting JWT token from PhotoShare auth bridge...');
        const bridgeToken = await window.getPhotoShareJwtTokenForAndroid();
        
        if (bridgeToken && bridgeToken !== 'null' && bridgeToken !== 'FUNCTION_NOT_FOUND') {
          console.log(`🔑 Got valid JWT token from bridge (length: ${bridgeToken.length})`);
          return bridgeToken;
        }
      }
  ...
  /**
   * Send JWT token to Android Event Picker using chunked transfer
   */
  async sendJwtTokenToAndroidEventPicker(jwtToken, chunkSize = 200) {
  ...
  /**
   * Send JWT token to Android (corrected red box handler)
   */
  async sendJwtTokenToRedBox(token) {
    ...
    try {
      // Use the chunked transfer method
      const success = await this.sendJwtTokenToAndroidEventPicker(token, 200);
  ...
  /**
   * Red button JWT test handler
   */
  async handleRedBoxJwtTest() {
    console.log('🔍 RED BOX: Starting JWT test with chunked transfer...');

    try {
      // Get the JWT token
      console.log('🔍 RED BOX: Getting JWT token...');
      const token = await this.getAuthToken();

      if (token && token !== 'null' && token !== 'FUNCTION_NOT_FOUND' && token !== 'mock_jwt_token_for_testing') {
        console.log('🔍 RED BOX: Got valid token, sending via chunked transfer: TOKEN_' + token.length + '_CHARS');
        
        // Use corrected chunked transfer
        const success = await this.sendJwtTokenToRedBox(token);
...
try {
  ...
  // Export JWT functions for red button testing
  window.handleRedBoxJwtTest = async function() {
    try {
      return await window.websiteIntegration.handleRedBoxJwtTest();
    } catch (error) {
      console.error('Error in handleRedBoxJwtTest:', error);
      return false;
    }
  };

  window.sendJwtTokenToRedBox = async function(token) {
    try {
      return await window.websiteIntegration.sendJwtTokenToRedBox(token);
    } catch (error) {
      console.error('Error in sendJwtTokenToRedBox:', error);
      return false;
    }
  };

  console.log('✅ WebsiteIntegration service created with JWT functions');
