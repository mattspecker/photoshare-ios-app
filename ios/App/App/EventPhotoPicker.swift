...
public class EventPhotoPicker: CAPPlugin, CAPBridgedPlugin {
    ...
    static func checkPhotoAlreadyUploaded(asset: PHAsset, eventId: String, completion: @escaping (Bool) -> Void) {
        ...
        AppDelegate.refreshJwtTokenIfNeeded { token in
            guard let jwtToken = token else {
                print("❌ No JWT token available for duplicate check")
                completion(false)
                return
            }
    ...
    private static func performBackgroundUpload(
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error {
                print("❌ Auto-upload network error: \(error)")
            ...
            print("📡 Auto-upload HTTP Status: \(httpResponse.statusCode)")
            
            if httpResponse.statusCode == 200 {
                print("✅ Auto-upload successful for photo: \(asset.localIdentifier)")
                completion(true, nil)
            } else {
    ...
    @objc func openEventPhotoPicker(_ call: CAPPluginCall) {
        ...
        if let token = jwtToken {
            print("✅ JWT token received from JavaScript: \(token.prefix(20))...")
        } else {
            print("⚠️ No JWT token received from JavaScript, trying AppDelegate fallback")
            // Fallback to AppDelegate if not provided
            jwtToken = AppDelegate.getStoredJwtToken()
            
            if jwtToken == nil || !AppDelegate.isJwtTokenValid() {
                print("⚠️ No valid JWT token in AppDelegate either")
                // Try to refresh
            ...
            if let token = jwtToken {
                print("🔐 Using fallback JWT token from AppDelegate: \(token.prefix(20))...")
            } else {
                print("⚠️ No JWT token available from any source")
            }
        }
        ...
        guard let startDateUTC = parseDate(startDateStr),
            ...
            print("❌ Received startDate: '\(startDateStr)'")
            print("❌ Received endDate: '\(endDateStr)'")
            print("❌ Expected formats: '2025-08-14T10:00:00Z' or '2025-08-14T10:00:00+00:00' or '2025-08-14T10:00:00.000Z'")
        ...
        for (index, uploadedPhoto) in uploadedPhotos.enumerated() {
            ...
            if let uploadedPerceptualHash = uploadedPhoto["perceptual_hash"] as? String {
                ...
                if let devicePerceptualHash = generatePerceptualHash(for: asset) {
                    ...
                    if similarity >= 0.90 {
                        print("✅ PERCEPTUAL MATCH: Found perceptual hash match for \(asset.localIdentifier)")
                        return true
                    } else {
                        print("⚠️ Perceptual hashes similar but below threshold: \(similarity)")
                    }
                } else {
                    print("❌ Failed to generate perceptual hash for device photo")
                }
            }
